...
(function(){
  ...
  // --- Compact mode toggle based on viewport ---
  ...
  // --- Simple swipe navigation (prev/next month) ---
  ...
  function addEntryFlow(date){
    ...
    if(amtStr===null) return false;
    const amt = Number(String(amtStr).replace(/[^0-9.\-]/g,''));
    if(!Number.isFinite(amt) || amt<=0){ alert("Please enter a positive number."); return false; }
    const arr = entries(date); arr.push({label, amount: Math.round(amt)}); setEntries(date, arr); return true;
  }
  ...
  function render(){
    ...
    for(let d=1; d<=days; d++){
      ...
      // Add a small mobile action row: "View" and "+ Add"
      ...
      viewBtn.addEventListener('click', (ev)=>{
        ev.stopPropagation();
        cell.classList.toggle('show-items');
      });

      // Mobile-friendly add: reuse prompt flow
      const addBtn = document.createElement('button');
      addBtn.type = 'button';
      addBtn.textContent = '+ Add';
      addBtn.addEventListener('click', (ev)=>{
        ev.stopPropagation();
        if(addEntryFlow(date)){ render(); }
      });
  ...
  // Enable swipe nav on touch devices (left: next month, right: previous month)
  addSwipeNav(document.getElementById("calendarGrid"),
  ...
  $("#exportBtn").addEventListener("click", ()=>{
    const payload = { exportedAt: new Date().toISOString(), settings, data: store };
